...
@Injectable()
export class PoliciesServiceOld {
  constructor(
  ...
  async *satisfiablePolicies(proposalId: string, queryArgs?: SelectManyPoliciesArgs) {
  ...
  async *satisifiedPolicies(proposalId: string, queryArgs?: SelectManyPoliciesArgs) {
    for await (const [policy, sat] of this.satisfiablePolicies(proposalId, queryArgs)) {
