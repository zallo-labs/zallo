...
export const ActivityTransfers = memo(
  ({ transfers, textStyle }: CallTokensProps) => {
    const styles = useStyles();

    const values = _.zip(
      transfers,
      useTokenValues(
        ...transfers.map((t): [Address, BigNumber] => [t.token.addr, t.amount]),
      ),
    );
          ...
          (t) =>
            !t.amount.isZero() && (
              <Text
                ...
                style={[
                  textStyle,
                  t.direction === TransferType.In && styles.positive,
                ]}
              >
