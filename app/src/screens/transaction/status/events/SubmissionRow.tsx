...
export const SubmissionRow = memo(({ submission }: SubmissionRowProps) => {
  ...
  const [content, Icon] = match<SubmissionStatus, [string, EventRowProps['Icon']]>(
    submission.status,
  )
    .with('pending', () => ['Submitted', SendIcon])
    .with('success', () => ['Executed', FinalizedIcon])
    .with('failure', () => ['Submission Failed', () => <ErrorIcon color={colors.error} />])
    .exhaustive();

  return <EventRow Icon={Icon} content={content} timestamp={submission.timestamp} />;
});
